...
public class TestKeyLoopView: NSView {
   var rootView: NSView?
   var keyViews: [TestKeyView] = []
   var rootFrame: NSRect?
   
   public static let kMainWindowFlagsChanged = "mainWindowFlagsChanged"
   
   public static var sharedTestKeyLoopView = TestKeyLoopView()
   ...
   public class func enableForView(_ view: NSView) {
      sharedTestKeyLoopView.rootView = view
      NotificationCenter.default.addObserver(sharedTestKeyLoopView, selector: #selector(mainWindowFlagsChanged), name: Notification.Name(rawValue: kMainWindowFlagsChanged), object: nil)
   }
   ...
   private func insertIntoViewHirarchy() {
   ...
   private func showNextKeyViewLoopFor(view startView: NSView) {
      nextKeyViewLoopFor(startView: startView) { $0?.nextKeyView }
   }

   private func showPreviousKeyViewLoopFor(view startView: NSView) {
   ...
   private func attributedFont(_ font: NSFont, color: NSColor) -> [NSAttributedStringKey: Any] {
